...
export default {
  ...
  data() {
    return {
      ...
      captchaUrl: 'http://115.235.130.55:3000/verifyCode',
      message: '',
      loading: false
    };
  },
  methods: {
    async login() {
      this.loading = true;
      try {
        const response = await axios.post('http://115.235.130.55:3000/api/login', {
        ...
        const socket = io('http://115.235.130.55:3000', {
    ...
    refreshCaptcha() {
      this.captchaUrl = `http://115.235.130.55:3000/verifyCode?${Date.now()}`;
    }
  }
};
</script>
