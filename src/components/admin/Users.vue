      ...
      <el-table :data="pagedUsers" :height="tableHeight" :key="tableKey">
        ...
        <el-table-column label="头像" min-width="100" align="center">
          <template v-slot="scope">
            <img :src="'http://115.235.142.254:3000' + scope.row.userface" alt="头像" class="userface">
          </template>
        </el-table-column>
...
export default {
  ...
  setup() {
    ...
    const fetchUsers = async (searchKeyword = '') => {
      try {
        const response = await axios.get('http://115.235.142.254:3000/api/users', {
    ...
    const handleDelete = (row) => {
      ElMessageBox.confirm('确定要删除这条记录吗？', '提示', {
        ...
        try {
          await axios.post('http://115.235.142.254:3000/api/deleteUser', { username: row.username });
          fetchUsers();
    ...
    const saveUser = async () => {
      try {
        if (isEdit.value && selectedFile.value) {
          ...
          const uploadResponse = await axios.post('http://115.235.142.254:3000/api/uploadAvatar', formData, {
            headers: {
        ...
        if (isEdit.value) {
          await axios.post('http://115.235.142.254:3000/api/updateUser', currentUser);
          ElMessage.success('更新成功');
        } else {
          await axios.post('http://115.235.142.254:3000/api/register', currentUser);
    ...
    const exportUsers = async (format) => {
      try {
        const response = await axios.get(`http://115.235.142.254:3000/api/exportUsers/${format}`, {
          responseType: 'blob'
        });
    ...
    const downloadTemplate = async (format) => {
      try {
        const response = await axios.get(`http://115.235.142.254:3000/api/downloadTemplate/${format}`, {
          responseType: 'blob'
        });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
    ...
    const importUsers = async (event) => {
      ...
      try {
        await axios.post(`http://115.235.142.254:3000/api/importUsers/${format}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data'
          }
        });
