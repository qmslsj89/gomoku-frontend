    ...
    <el-card class="box-card">
      <div class="info-section">
        ...
        <div v-else class="current-player">
          ...
          <el-button v-if='!isPlayingWithBot' type="primary" @click="playWithBot" class="play-with-bot-button">与机器人对局</el-button>
        </div>
      ...
      <div class="game-history" v-if="showGameHistory">
        <h2>对局记录</h2>
...
export default {
  ...
  methods: {
    ...
    fetchGameHistory() {
      ...
      axios.get('http://115.235.134.18:3000/api/me', { headers: { 'x-access-token': token } })
        .then(response => {
          ...
          return axios.get(`http://115.235.134.18:3000/api/games/${username}`);
        })
        .then(response => {
    ...
    handleCellClick(row, col) {
      if (this.board[row][col] === 0 && this.currentPlayer === this.getCurrentPlayerIndex()) {
        ...
        if (socket) {
          ...
          socket.emit('gameMessage', { type: 'move', row, col, player: this.currentPlayer });
        }
      }
    },
    ...
    recordGame(winnerId) {
      ...
      axios.get('http://115.235.134.18:3000/api/me', { headers: { 'x-access-token': token } })
        .then(()=> {
          console.log('当前用户信息:', this.players);
          
          console.log('玩家1:', this.players[0]);
          console.log('玩家2:', this.players[1]);
          console.log('胜者ID:', winnerId);
      ...
      if (socket) {
        setInterval(() => {
          ...
          socket.emit('ping', () => {
            ...
            socket.emit('latency', { player: this.currentPlayer, latency });
          });
        }, 1000); // 每秒检查一次延迟
      }
    },
