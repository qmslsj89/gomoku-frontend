            ...
            <el-dropdown @command="handleCommand">
              <template #default>
                <div class="user-info animated-user-info">
                  ...
                  <img :src="'http://115.235.157.65:3000' + userface" alt="用户头像" class="user-avatar" fit="cover">
                </div>
              </template>
              <template #dropdown>
                <el-dropdown-menu>
    ...
    <el-dialog v-model="isProfileDialogVisible" title="个人信息" :width="dialogWidth">
      <el-form :model="editableUser" label-width="100px">
        <el-form-item label="头像">
          <el-upload
            action="http://115.235.157.65:3000/api/uploadAvatar"
          ...
          >
            <img v-if="editableUser.userface" :src="'http://115.235.157.65:3000' + editableUser.userface" class="user-avatar" />
          </el-upload>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    logout() {
      axios.post('http://115.235.157.65:3000/api/logout')
        .then(() => {
    ...
    async saveUserInfo() {
      try {
        const response = await axios.post('http://115.235.157.65:3000/api/updateUser', this.editableUser);
    ...
    async deleteAccount() {
      try {
        const response = await axios.post('http://115.235.157.65:3000/api/deleteUser', { username: this.editableUser.username });
        if (response.data.success) {
          localStorage.removeItem('token');
          localStorage.removeItem('username');
          this.isLoggedIn = false;
    ...
    async fetchUserInfo() {
      ...
      if (token) {
        try {
          const response = await axios.get('http://115.235.157.65:3000/api/me', { headers: { 'x-access-token': token } });
    ...
    async sendMessage() {
      ...
      try {
        const response = await fetch('http://115.235.157.65:3000/api/MyAi', {
          method: 'POST',
          headers: {
